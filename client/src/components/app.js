...
class App extends Component {
  ...
  componentWillMount() {
    ...
    if (loc && loc !== 'signin') {
      axios.get(`/api/search?location=${loc}`,
        {
          headers: {
  ...
  handleSubmit() {
    const locationText = this.state.searchBarText.trim().length === 0 ? 'USA' : this.state.searchBarText;
    axios.get(`/api/search?location=${locationText}`,
      {
        headers: {
          authorization: localStorage.getItem('token') || '',
          'Cache-control': 'no-cache'
  ...
  renderCards() {
    if (this.state.places && this.state.places.length) {
      return <PlaceCards places={this.state.places} cardClicked={this.cardClicked} />
    }
  }
        ...
        <div className='app-body'>
          <div>
            <form onSubmit={e => {
              ...
              <button type='submit'><i className="fa fa-search fa-3x" aria-hidden="true"></i></button>
            </form>
          </div>
          {this.renderCards()}
        </div>
